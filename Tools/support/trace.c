...
/*  Amazingly, this generates a redefinition error under Norcroft 5.52
#ifdef TRACE
#undef TRACE
#define TRACE 1
#endif
...
#if DEBUG
   fprintf( stderr, "%s %d trace_f: \"%s\"\n",
   		__FILE__, __LINE__, has_format ? format : ""  );
#endif
      ...
      if (file != NULL)
      {
         ...
         switch (Trace)
         {
          ...
          case TUBE:
            ...
            case VDU:
#if DEBUG
   		fprintf( stderr, "%s %d trace_f VDU: \"%s\"\n",
      ...
      if (has_format)
      {
         ...
         switch (Trace)
         {
          ...
          case TUBE:
            ...
            case VDU:
#if DEBUG
   		fprintf( stderr, "%s %d trace_f: VDU\n", __FILE__, __LINE__ );
#endif
               for (cc = s; *cc != '\0'; cc++)
               {
                  char c = *cc;
...
finish:
   if (error != NULL)
   {
      Error = error;
#if DEBUG
	fprintf( stderr, "%s %d trace_f Error: \"%s\"\n",
		__FILE__, __LINE__, error -> errmess );
#endif
   }
}
/*------------------------------------------------------------------------*/
