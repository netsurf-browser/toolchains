...
#define EXPANSION_BASE_NAMEExpansionBase
#endif
...
#define AddBootNode(bootPri, flags, deviceNode, configDev) ({ \
  ...
  APTR _AddBootNode_deviceNode = (deviceNode); \
  ...
  register APTR __AddBootNode_deviceNode __asm("a0") = (_AddBootNode_deviceNode); \
...
#define ReadExpansionByte(board, offset) ({ \
  ...
  register CONST_APTR __ReadExpansionByte_board __asm("a0") = (_ReadExpansionByte_board); \
...
#define WriteExpansionByte(board, offset, byte) ({ \
  ...
  UBYTE _WriteExpansionByte_byte = (byte); \
  { \
  ...
  register UBYTE __WriteExpansionByte_byte __asm("d1") = (_WriteExpansionByte_byte); \
...
#define MakeDosNode(parmPacket) ({ \
  ...
  register CONST_APTR __MakeDosNode_parmPacket __asm("a0") = (_MakeDosNode_parmPacket); \
...
#define WriteExpansionWord(board, offset, word) ({ \
  APTR _WriteExpansionWord_board = (board); \
  ULONG _WriteExpansionWord_offset = (offset); \
  UWORD _WriteExpansionWord_word = (word); \
  { \
  register struct ExpansionBase * const __WriteExpansionWord__bn __asm("a6") = (struct ExpansionBase *) (EXPANSION_BASE_NAME);\
  register APTR __WriteExpansionWord_board __asm("a0") = (_WriteExpansionWord_board); \
  register ULONG __WriteExpansionWord_offset __asm("d0") = (_WriteExpansionWord_offset); \
  register UWORD __WriteExpansionWord_word __asm("d1") = (_WriteExpansionWord_word); \
  __asm volatile ("jsr a6@(-162:W)" \
  : \
  : "r"(__WriteExpansionWord__bn), "r"(__WriteExpansionWord_board), "r"(__WriteExpansionWord_offset), "r"(__WriteExpansionWord_word) \
  : "d0", "d1", "a0", "a1", "fp0", "fp1", "cc", "memory"); \
  } \
})

#define GetMachineInfo(tagList) ({ \
  const struct TagItem * _GetMachineInfo_tagList = (tagList); \
  ULONG _GetMachineInfo__re = \
  ({ \
  register struct ExpansionBase * const __GetMachineInfo__bn __asm("a6") = (struct ExpansionBase *) (EXPANSION_BASE_NAME);\
  register ULONG __GetMachineInfo__re __asm("d0"); \
  register const struct TagItem * __GetMachineInfo_tagList __asm("a0") = (_GetMachineInfo_tagList); \
  __asm volatile ("jsr a6@(-168:W)" \
  : "=r"(__GetMachineInfo__re) \
  : "r"(__GetMachineInfo__bn), "r"(__GetMachineInfo_tagList) \
  : "d1", "a0", "a1", "fp0", "fp1", "cc", "memory"); \
  __GetMachineInfo__re; \
  }); \
  _GetMachineInfo__re; \
})

#ifndef NO_INLINE_STDARG
static __inline__ ULONG ___GetMachineInfoTags(struct ExpansionBase * ExpansionBase, Tag tag1, ...)
{
  return GetMachineInfo((const struct TagItem *) &tag1);
}

#define GetMachineInfoTags(...) ___GetMachineInfoTags(EXPANSION_BASE_NAME)
#endif
